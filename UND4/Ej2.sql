...
-- 4. Mostrar jugadores de un equipo con ID conocido (por ejemplo, equipos de Madrid).
    select nombre_jugador from jugadores where id_equipo =9;
...
-- 6. Ordenar equipos por ciudad alfabéticamente.
    select ciudad from equipos order by ciudad ; ("repitiendo ciudades si las hay repetidas")
    select distinct ciudad from equipos order by ciudad ; ("sin repetir ciudades si las hay repetidas")

-- 7. Filtrar jugadores que juegan como "Delantero" y tienen más de 20 goles.
    select nombre_jugador,posicion from jugadores where posicion="Delantero" and goles>20;

-- 8. Obtener el promedio de goles por partido para un equipo con un ID específico.
    select avg(goles_local+goles_visitante) from partidos where id_equipo_local=9 or id_equipo_visitante = 9;
...
-- 11. Ordenar equipos por ciudad alfabéticamente y despues por año de fundacion descendente.
    select * from equipos order by ciudad, fundado;
